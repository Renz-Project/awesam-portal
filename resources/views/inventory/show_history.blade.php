        ...
        <div class="modal-dialog-scrollable modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-success" id="inventory{{$key}}Label">
                        Stock Movement ({{ $row['product_name'] }}) - {{ $row['location'] }}
                    </h5>
                            ...
                            <tbody>
                                ...
                                @foreach($row['stockMovements'] as $transaction)
                                    <tr>
                                        ...
                                        <td>
                                            <span @if($transaction->type == "inflow") class="text-success" @else class="text-danger" @endif>
                                        ...
                                        @if(auth()->user()->role == 'Super Admin')
                                            <td>
                                                <button 
                                                    class="btn btn-sm btn-warning editMovementBtn"
                                    ...
                                    @php
                                        $total += ($transaction->type == "inflow") ? $transaction->quantity : -$transaction->quantity;
                                    @endphp
                                @endforeach
                                ...
                                @foreach($row['transactions'] as $transaction)
                                    ...
                                    @php $total -= $transaction->qty; @endphp
                                @endforeach

                                <tr>
                                    <td colspan="{{ auth()->user()->role == 'Super Admin' ? 5 : 4 }}" class="text-right">Total</td>
